<x-layout title="Saved recipes">
    <x-slot name="header">
        ...
        <p class="mt-1 text-sm text-gray-600">
            ...
            @if ($activeTag)
                ...
                @if ($search !== '')
                    and matching <span class="font-medium text-orange-600">"{{ $search }}"</span>
                @endif
            @elseif ($search !== '')
    ...
    <div class="mb-6 rounded-lg bg-white p-4 shadow-sm ring-1 ring-gray-200">
        <form method="GET" action="{{ route('saved-recipes.index') }}" class="flex flex-wrap items-center gap-3">
            ...
            @if ($search !== '' || $activeTag)
                <a href="{{ route('saved-recipes.index') }}" class="text-sm font-medium text-gray-600 hover:text-gray-900">Clear</a>
            @endif
        </form>

        @if ($tags->isNotEmpty())
            <div class="mt-4 flex flex-wrap gap-2 border-t border-gray-100 pt-4">
                <a href="{{ route('saved-recipes.index', ['q' => $search]) }}"
                    @class([
                ...
                @foreach ($tags as $tag)
                    <a href="{{ route('saved-recipes.index', ['tag' => $tag->slug, 'q' => $search]) }}"
                        @class([
    ...
    @if ($recipes->isEmpty())
        @if ($search !== '' || $activeTag)
            <div class="rounded-lg border-2 border-dashed border-gray-300 p-12 text-center">
                <h3 class="text-sm font-semibold text-gray-900">No matches</h3>
                <p class="mt-1 text-sm text-gray-500">None of your saved recipes match the current filters.</p>
                <a href="{{ route('saved-recipes.index') }}" class="mt-4 inline-flex items-center gap-1.5 rounded-md bg-white px-3 py-2 text-sm font-medium text-gray-700 shadow-sm ring-1 ring-gray-300 hover:bg-gray-50">
                    Clear filters
                </a>
            </div>
        @else
            <div class="rounded-lg border-2 border-dashed border-gray-300 p-12 text-center">
                <h3 class="text-sm font-semibold text-gray-900">No saved recipes yet</h3>
                <p class="mt-1 text-sm text-gray-500">Tap the Save button on any recipe to bookmark it.</p>
                <a href="{{ route('recipes.index') }}" class="mt-4 inline-flex items-center gap-1.5 rounded-md bg-orange-500 px-3 py-2 text-sm font-medium text-white hover:bg-orange-600">
                    Browse recipes
                </a>
            </div>
        @endif
